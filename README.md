...
The second phase shifted from infrastructure to analytics, focusing on extracting business value from the engineered environment. Using the curated gold layer, I conducted exploratory data analysis and developed advanced SQL analyses to answer key business questions related to growth, customer behavior, and product performance. These analyses revealed several high-impact insights, including strong revenue concentration in a small number of geographic regions, a clear distinction between high-volume and high-margin product categories, confirmation of an 80/20 revenue distribution across the product catalog, and the identification of a high-value VIP customer segment with significantly higher retention rates.
...
*(Placeholder for interactive links to all sections below)*
...
As a result of these challenges, business teams spent significant time trying to understand messy numbers instead of analyzing them. Leadership lacked confidence in performance reports, marketing decisions were based on incomplete regional insights, and product strategy relied more on intuition than data. Therefore, the objective of this project was to move the organization from manual, fragmented reporting to a centralized and automated analytics environment. The solution focused on building a scalable data foundation that could support consistent reporting and analysis across the business. Key objectives included:

**Establish an Authoritative Data Source:** Centralize CRM and ERP data into a single SQL data warehouse so all teams operate from the same validated metrics.

**Implement Medallion Architecture:** Design a scalable three tier pipeline (Bronze, Silver, Gold) to systematically transform raw data into business ready assets.

**Automate ETL Processes:** Replace manual data handling with automated stored procedures and bulk loading to ensure data is consistently refreshed and prepped for analysis.

**Optimize for BI and Analytics:** Implement a star schema using fact and dimension tables to support efficient querying for use cases such as year-over-year growth, customer segmentation, and performance analysis.
...
**Silver Layer (Cleansed & Standardized):** responsible for enforcing data quality and consistency across the warehouse. At this stage, raw data is refined through schema enforcement, NULL handling, categorical standardization (such as gender and status codes), and normalization of date formats. Duplicate records across integrated systems are resolved using SQL window functions, producing a clean and standardized dataset. This layer prevents data quality issues from growing and affecting downstream reporting.
...
One of the main challenges in this project was integrating two systems that did not share a common primary key structure. To resolve this, I designed a relationship model that maps transactional records from the CRM to corresponding master data in the ERP using standardized business keys and transformation logic. This integration layer creates a unified analytical view of customers, products, and sales activity while preserving data integrity across systems.

This unified integration eliminated the need for manual CSV joins and significantly reduced reporting turnaround time, allowing the company to analyze customers, products, and sales activity in a consistent view.
...
*[INSERT GOLD LAYER DIAGRAM HERE]*
...
**Semantic Layer:** All fields were renamed from technical shorthand to appropriate business aliases (e.g., cst_id to Customer_ID). This semantic layer improves usability for non-technical stakeholders and ensures that the data model is aligned with business terminology.
...
**Database Exploration:** A high-level audit of the schema to confirm table counts, column data types, and the presence of primary and foreign keys. This step validates the physical health of the warehouse.

**Dimensions Exploration:** An investigation into categorical fields (e.g., Gender, Category, Status). Used DISTINCT counts to identify all valid groups and ensure there were no hidden or misspelled categories that could skew reporting.

**Date Exploration:** Determined the boundaries of the data using MIN() and MAX() on order dates to identify the full coverage period (e.g., 2022–2024) and detect any gaps in transaction history.

**Measures Exploration:** Profiled the numerical facts of the business by calculating total sales, average product costs, and item counts using SUM(), AVG(), and COUNT() to establish business scale.

**Magnitude Analysis:** Analyzed [Measure] BY [Dimension] (e.g., total sales by product category) to identify which segments drive the majority of business volume.

**Ranking Analysis:** Used ORDER BY and window functions such as RANK() and DENSE_RANK() to isolate top and bottom performers, identifying best-selling products and highest-value customers.
...
The EDA phase used a range of analytical techniques to explore, validate, and summarize the data. These techniques enabled efficient profiling of business dimensions and measures, with key techniques used including:

**Aggregations:** Used SUM(), AVG(), COUNT(), MIN(), and MAX() to calculate core KPIs and establish baseline business performance.

**Segmentation:** Applied GROUP BY and HAVING to segment measures by key dimensions and filter aggregated results for targeted analysis.

**Time-Series Analysis:** Leveraged DATEPART() and DATETRUNC() to analyze revenue trends and performance patterns across time.

**Ranking & Window Functions:** Used RANK() and DENSE_RANK() to identify top and bottom performing customers, products, and regions without losing record level detail.

---

## 8. Advanced Analytics and Reporting

The final phase of the project focuses on transforming the warehouse data into data that is ready for business analytics. While earlier phases focussed on data quality and uncovered baseline insights, this stage moves beyond reporting to answer important business questions related to growth, customer behavior, and product performance over time. This analytical layer is designed to support strategic decision making by tracking business momentum, benchmarking current performance against historical trends, and identifying high-impact customer and product segments. The analyses were developed around five core business themes, each aligned with stakeholder priorities:

**Growth Analysis:** Measured Year-over-Year (YoY) and Month-over-Month (MoM) revenue growth to evaluate long-term trends and short-term demand fluctuations.

**Cumulative Performance:** Implemented running totals for revenue and customer acquisition to visualize how the business scales throughout the fiscal year and tracks progress toward annual targets.

**Performance Benchmarking:** Compared current performance against historical averages and prior periods to identify breakout months and underperforming periods in proper context.

**Part-to-Whole Analysis:** Evaluated how individual products and categories contribute to total revenue, supporting 80/20 analysis for inventory and catalog optimization.

**Customer Segmentation:** Segmented customers based on purchase frequency, recency, and total spend to differentiate high-value customers from standard and at risk segments.

The analyses uncovered several strategic insights that have direct business implications that can be used by the company. The insights included business topics such as growth insights, revenue and product performance, and customer behaviour:
...
The analytics process took the data foundation and turned it into measurable business outcomes. By applying time-based analysis, segmentation, and performance benchmarking, the project uncovered insights that directly inform strategic decision-making across revenue growth, product optimization, and customer retention. To complete the analyses, following techniques were used:

**Common Table Expressions (CTEs):** Used to modularize complex, multi-step logic and improve query readability.
...
**Complex Joins and Subqueries:** Integrated fact and dimension tables to ensure all analyses were enriched with full business context, including geography and product hierarchy.

The final product of this phase was the creation of numerous SQL views in the gold layer. These views serve as the final consumable outputs of the warehouse. These views are designed for direct integration with BI tools such as Power BI or Tableau:
...
Maintaining high-quality data was an important requirement throughout the project. A structured validation framework was implemented across the medallion architecture to detect issues early and prevent invalid data from reaching the gold layer. By enforcing quality checks at each stage of the pipeline, the warehouse delivers reliable data throughout the pipeline. The validation approach relies on automated QA scripts that evaluate predefined data quality rules and return clear pass/fail outcomes. Exception reporting queries isolate invalid records for fast troubleshooting and root-cause analysis. In addition, core validation logic is included directly into ETL load procedures, allowing issues to be flagged during pipeline execution rather than after reporting discrepancies occur. These can be summarized as follows:
...
Overall, the project was successfully able to deliver a well performing SQL data warehouse by integrating CRM and ERP systems into a single platform. From the company's perspective, this project replaced fragmented reporting with a unified analytics platform that supports faster, more confident decision making across departments. Using a medallion architecture, the pipeline enables a flow from raw ingestion to a usable star schema for the business, supported by automated ETL processes and thorough data quality controls. Through the gold layer and advanced analytics section, several high-impact business insights were uncovered for the company:
...
Looking ahead, the project can easily be scaled and improved for further growth. Future enhancements could include introducing workflow orchestration with Apache Airflow, managing transformations as code using dbt, and migrating to a cloud-native platform such as Snowflake. With a clean and standardized data foundation already in place, the warehouse can also support advanced analytics use cases such as customer churn prediction and demand forecasting. 
